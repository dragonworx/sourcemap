...
export function AffineView(props: AffineViewProps) {
   ...
   const onMouseDown = (e: MouseEvent) => {
      ...
      setIsMouseDown(true);
      drag.start({x: screenX, y: screenY }, { x: panX, y: panY });
   ...
   const onMouseMove = (e: MouseEvent) => {
      if (isMouseDown && isPanHover) {
         ...
         const { x: newPanX, y: newPanY } = drag.value({x: screenX, y: screenY });
         setPanX(newPanX);
         setPanY(newPanY);
   ...
   const transformedStyle = {
      transition: isMouseDown ? 'none' : 'transform 0.5s ease-out',
